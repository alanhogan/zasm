...
The first major goal is maintainability — important whether your project involves one person or ten. For any sort of styling you want to do, under ZASM, there should be one clear, ‘right’ way to write the selector for it; this makes the CSS easy to author. Those rules should result in selectors that are easily and instantly ‘reverse engineered’ into the rule that created them and thus what their intentions are. This provides readability. By keeping the methodology small and tightly defined, new collaborators won’t face a large uphill climb to productivity — at least, assuming they have already learned enough CSS, an admittedly complicated pre-requisite!
